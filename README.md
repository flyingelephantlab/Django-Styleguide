...
- [Celery](#celery)
  ...
  * [Triggering a task](#triggering-a-task)
...
- Don’t but business logic inside a task
- task name is the same as service name
- task body:
    - fetch required data
    - import service inside task
    - call service
 
### Error handling

    - fire error method using on_failure callback
    - failure method body:
        - fetch args
...
### Triggering a task
    ...
    - import the task on module level and rename it to end with `_task`
    - fire it as a side-effect on transaction commit
